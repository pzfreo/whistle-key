...
def test_project_preserves_meshes_and_material_plates(tmp_path, dry_run):
    output = tmp_path/'project.3mf'
    source = ROOT/'build'
    package(source, output, dry_run)
    report = json.loads(output.with_suffix('.json').read_text())
    assert len(report) == 6
    assert sum(r['source'] == 'clamp_cap_print.stl' for r in report) == 2
